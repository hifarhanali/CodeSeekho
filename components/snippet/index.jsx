...
const Snippet = ({ snippetData }) => {
    ...
    const [snippet, setSnippet] = React.useState(snippetData)

    const redirectToPostPage = (e) => {
        e.preventDefault()
        if (snippet.id) {
            window.location.href = `/post/${snippet.id}`
        }
    }
                    ...
                    <div className="max-w-2xl mx-auto bg-primary-50 shadow-lg rounded-lg h-200">
                        <div className="px-6 py-5">
                            <div className="flex items-start">
                                ...
                                <img className="w-10 h-10 rounded-full mr-4" src={snippet.user.photo} alt="avatar" />
                                <div className="flex-grow truncate">

                                    <div className="w-full sm:flex justify-between items-center mb-3">

                                        <h2 className="text-2xl leading-snug font-extrabold text-gray-50 truncate mb-1 sm:mb-0 hover:cursor-pointer" onClick={redirectToPostPage}>{snippet.name}</h2>
                                        ...
                                        <div className="flex-shrink-0 flex items-center space-x-3 sm:ml-2">
                                            <button className="flex items-center text-left text-sm font-medium text-indigo-100 hover:text-text-light group focus:outline-none focus-visible:border-b focus-visible:border-text-light">
                                                <svg onClick={handleLikeClick} className="w-4 h-4 flex-shrink-0 mr-2 fill-current text-gray-300 group-hover:text-gray-200" viewBox="0 0 16 16">
                                                    <path d="M14.682 2.318A4.485 4.485 0 0 0 11.5 1 4.377 4.377 0 0 0 8 2.707 4.383 4.383 0 0 0 4.5 1a4.5 4.5 0 0 0-3.182 7.682L8 15l6.682-6.318a4.5 4.5 0 0 0 0-6.364Zm-1.4 4.933L8 12.247l-5.285-5A2.5 2.5 0 0 1 4.5 3c1.437 0 2.312.681 3.5 2.625C9.187 3.681 10.062 3 11.5 3a2.5 2.5 0 0 1 1.785 4.251h-.003Z" />
                                                </svg>

                                                <span                                                >{snippet.likes} <span className="sr-only">likes</span></span>
                                            </button>
                                            <button className="flex items-center text-left text-sm font-medium text-indigo-100 hover:text-text-light group focus:outline-none focus-visible:border-b focus-visible:border-text-light">
                                                ...
                                                <span>{snippet.reviews?.length} <span className="sr-only">comments</span></span>
                                            </button>
                                        </div>
                                    </div>
